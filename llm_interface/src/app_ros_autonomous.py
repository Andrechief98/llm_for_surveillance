...
class ChatNode():
    ...
    def __init__(self):
        ...
        print("Autonomous prompt considered")
    ...
    ## gestire l'esecuzione della run dove si aggiunge all contesto dell'LLM il messaggio ricevuto e si esegue la funzione per l'ottenimento delle strategie
        ...
        if req:
            ...
            if self.last_run == None or self.last_run.status == 'completed':
                ...
                # To allow to obtain the plan and then perform the actions, we send an automatic message to the LLM after the plan proposal:
                self.client.beta.threads.messages.create(
                    thread_id=self.thread.id,
                    role="user",
                    content="Execute the proposed plan"
                )
                
                start_response_time = time.time()

                # run per far eseguire le actions all’LLM
                self.last_run = self.client.beta.threads.runs.create_and_poll(
                    thread_id=self.thread.id,
                    assistant_id=self.assistant.id,
                    model=self.model_to_use,
                ...
                if self.last_run.status == 'completed':
                    ...
                    while self.last_run.status == 'requires_action':
                        # print("List of all function calls:")
                        # print(self.last_run.required_action.submit_tool_outputs.tool_calls)
                        ...
                        for tool in self.last_run.required_action.submit_tool_outputs.tool_calls:
                            ...
                            #     # Function execution
                            #     # result = self.send_robot_to_area(args["robot_to_send"], args["area_to_reach"], args["ros_topic"])
                            #     result = self.send_robot_to_area(args["robot_to_send"], args["area_to_reach"])
                            ...
                            if tool.function.name == "send_robots_to_area":
                                ...
                                # Function execution
                                # result = self.send_robot_to_area(args["robot_to_send"], args["area_to_reach"], args["ros_topic"])
                                result = self.send_robots_to_area(args["robots_sequence"])
                                ...
                                # Function execution
                                result = self.display_cameras(args["cameras_names_list"])
                                ...
                                # Function execution
                                result = self.reset_sensors_activation()
                                ...
                                # Function execution
                                result = self.control_actuator(args["actuators_sequence"])
                                ...
                                # Function execution
                                result = self.retrieve_system_state()
